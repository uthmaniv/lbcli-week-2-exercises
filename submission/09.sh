# Create a transaction with RBF enabled so the fee can be bumped later.
# Send 20,000,000 satoshis to 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP
# using the UTXOs from the transaction below.
raw_tx=""
txid=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.txid')
bitcoin-cli -regtest createrawtransaction \
  '[{"txid":"'"$txid"'","vout":0,"sequence":4294967293},{"txid":"'"$txid"'","vout":1,"sequence":4294967293}]' \
  '{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":0.20000000}'
