...
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value' | awk '{printf "%.0f", $1 * 100000000}')
...
# Extract the available UTXOs from the decoded transaction for spending
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=0
check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value' | awk '{printf "%.0f", $1 * 100000000}')
...
# Calculate the approximate transaction size and fee
...
# Create the input JSON structure with RBF enabled
# RBF is enabled by setting the sequence number to less than 0xffffffff-1
PAYMENT_ADDRESS="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
CHANGE_ADDRESS="bcrt1qg09ftw43jvlhj4wlwwhkxccjzmda3kdm4y83ht"

# Create a proper input JSON for createrawtransaction
...
# Extract and verify the key components from the decoded transaction
...
# Create a simple transaction that sends funds to the test address
...
# Create the input JSON structure with a 10-block relative timelock
...
# Calculate the amount to send (use the output value from SECONDARY_TX, minus a fee)
# Extract the output value from the secondary TX first
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX" | jq '.vout[0].value' | awk '{printf "%.0f", $1 * 100000000}')
...
# Create the raw transaction with timelock
